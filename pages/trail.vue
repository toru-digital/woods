<template>
	<section class="flex flex-col w-full h-full flex-col">
		<div class="w-full h-16 grid place-items-center">
			<p class='w-full p-8 text-lg'>{{ selected_tree ? selected_tree.title : '' }}</p>
		</div>
		...
		<div>
			<div class="w-full h-32 text-white flex flex-col">
				<p class="w-full text-center">{{ selected_tree == null ? '?' : getDistance ($store.state.latitude, $store.state.longitude, selected_tree.lat, selected_tree.lon)}} meters</p>
				<div class="w-full flex flex-row justify-between flex-1 grid grid-cols-2">
